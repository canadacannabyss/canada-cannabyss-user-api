...
const emailResetPasswordTemplate = require('../templates/emailResetPassword');
...
module.exports = async (emailTo, userId) => {
  ...
  jwt.sign({ userId }, process.env.RESET_PASSWORD_SECRET, {
    ...
    const url = `${process.env.EMAIL_VERIFICATION_ACCOUNT_FRONTEND}/reset-password/${emailToken}`;

    const info = await transporter.sendMail({
      from: `"Canada Cannabyss" <${process.env.EMAIL_SMTP_USERNAME}>`, // sender address
      to: `${emailTo}`, // list of receivers
      subject: 'Password Reset - Canada Cannabyss', // Subject line
